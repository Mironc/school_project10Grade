...
vec3 depth_to_world_pos(float depth, vec2 uv) {
    ...
    vec4 worldSpacePosition = inv_proj * clipSpacePosition;

            // Perspective division
    worldSpacePosition /= worldSpacePosition.w;
    worldSpacePosition = inv_view * worldSpacePosition;
...
void main() {
    ...
    float attenuation = (1.0 / (1.0+D*D)) * light_props[instance].light_power;
    ...
    vec3 diffuse_result = vec3(max(dot(frag_normal, light_direction), 0.0));

    vec3 view_direction = normalize(camera_position - frag_position);

    vec3 half_dir = normalize(light_direction + view_direction);
    vec3 specular_result = vec3(specular * pow(clamp(dot(frag_normal, half_dir), 0.0, 1.0), shininess));

    frag_color = vec4((diffuse_result + specular_result) * (light_props[instance].light_color.rgb * attenuation) * color, 1.0);
    //frag_color = vec4(sqrt(vec3(D)),1.0);
    //frag_color = vec4(vec3(sqrt(log2(length(frag_position-camera_position)))),1.0);
}